...
export async function setUserState(userId, step, payload = {}) {
    return User.findOneAndUpdate({ telegramUserId: userId }, { $set: { state: { step, payload } } }, { new: true });
}
export async function setDefaultCategory(userId, categoryId) {
    return User.findOneAndUpdate({ telegramUserId: userId }, { $set: { defaultCategoryId: categoryId } }, { new: true });
}
...
export async function clearDefaultCategory(userId) {
    return User.findOneAndUpdate({ telegramUserId: userId }, { $set: { defaultCategoryId: null } }, { new: true });
}
