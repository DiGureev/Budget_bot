...
import { countActiveCategories, createCategory, getActiveCategories, getCategoryById, applyAmount, archiveCategory, renameCategory, updateCategoryBudget, resetCategorySpend, convertAnnualToMonthly, } from '../services/categoryService.js';
import { setDefaultCategory, clearDefaultCategory, getDefaultCategoryById, } from '../services/userService.js';
import { getCategoryButtonLabel, categoriesReplyKeyboard, categoryActionsKeyboard, defaultChoiceKeyboard, editCategoryKeyboard, confirmRemoveKeyboard, } from './keyboards.js';
import { formatCategoryDetails, formatMonthlyHistory, formatAnnualHistory, formatRemovePrompt, formatMoney, } from './messages.js';
import { WELCOME_MESSAGE, ACCOUNT_CREATED_MESSAGE, ADD_NEW_CATEGORY_MESSAGE, EMAIL_VALIDATION_ERROR, EMAIL_SAVE_ERROR, BOT_STARTED_MESSAGE, ADD_CATEGORY_ERROR, ENTER_CATEGORY_NAME_MESSAGE, CATEGORY_NAME_EXISTS_ERROR, CREATE_CATEGORY_ERROR, CATEGORY_TYPE_CHOICE_MESSAGE, CATEGORY_BUDGET_VALIDATION_ERROR, CATEGORY_CREATED_MESSAGE, MAKE_DEFAULT_CATEGORY_MESSAGE, DEFAULT_CATEGORY_SET_MESSAGE, AMOUNT_VALIDATION_ERROR, CATEGORY_NOT_FOUND_ERROR, NOT_DEFAULT_CATEGORY_ERROR, HELP_MESSAGE } from './constants.js';
...
export async function handleText(bot, msg, user) {
    ...
    if (user.onboarding.completed && !hasCategories && step === null && text !== ADD_NEW_CATEGORY_MESSAGE) {
        await bot.sendMessage(msg.chat.id, 'Please create categories to track your spendings first.', { parse_mode: 'HTML' });
        return;
    }
    if (step === 'awaiting_email') {
        ...
        if (!result.ok) {
            await bot.sendMessage(msg.chat.id, EMAIL_SAVE_ERROR);
            return;
        }
        ...
        await user.save();
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.sendMessage(msg.chat.id, ACCOUNT_CREATED_MESSAGE, { parse_mode: 'HTML', ...categoryKeyboardOptions(categories, user) });
        return;
    }
    if (step === 'awaiting_default_category_confirm') {
        await bot.sendMessage(msg.chat.id, 'Please choose Yes or No using the buttons below.');
        return;
    }
    if (text === ADD_NEW_CATEGORY_MESSAGE) {
        const count = await countActiveCategories(user.telegramUserId);
        if (count >= 8) {
            await bot.sendMessage(msg.chat.id, ADD_CATEGORY_ERROR, { parse_mode: 'HTML', ...categoryKeyboardOptions(activeCategories, user) });
            return;
        }
    ...
    const selectedCategory = activeCategories.find((category) => text === getCategoryButtonLabel(category, user));
    if (selectedCategory) {
        user.state = {
        ...
        await user.save();
        await bot.sendMessage(msg.chat.id, formatCategoryDetails(selectedCategory), {
    ...
    if (step === 'awaiting_category_name') {
        ...
        if (count >= 8) {
            ...
            await user.save();
            const categories = await getActiveCategories(user.telegramUserId);
            await bot.sendMessage(msg.chat.id, ADD_CATEGORY_ERROR, { parse_mode: 'HTML', ...categoryKeyboardOptions(categories, user) });
            return;
        }
        const normalizedName = normalizeCategoryName(text);
        const existing = activeCategories.some((category) => category.name === normalizedName);
        if (existing) {
            await bot.sendMessage(msg.chat.id, CATEGORY_NAME_EXISTS_ERROR, { parse_mode: 'HTML' });
            return;
        }
        user.state = {
    ...
    if (step === 'awaiting_category_budget') {
        ...
        if (amount === null || amount < 0) {
            await bot.sendMessage(msg.chat.id, CATEGORY_BUDGET_VALIDATION_ERROR, { parse_mode: 'HTML' });
            return;
        }
        ...
        try {
            ...
            await user.save();
            const categories = await getActiveCategories(user.telegramUserId);
            await bot.sendMessage(msg.chat.id, CATEGORY_CREATED_MESSAGE(category.name, formatMoney(amount), 'annual'), { parse_mode: 'HTML', ...categoryKeyboardOptions(categories, user) });
            return;
        }
        catch {
    ...
    if (step === 'awaiting_category_amount') {
        const amount = parseAmount(text);
        if (amount === null) {
            await bot.sendMessage(msg.chat.id, AMOUNT_VALIDATION_ERROR, { parse_mode: 'HTML' });
            return;
        }
        const categoryId = String(user.state.payload.categoryId ?? '');
        ...
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.sendMessage(msg.chat.id, formatCategoryDetails(category), {
            ...categoryKeyboardOptions(categories, user)
        });
        return;
    }
    if (step === 'awaiting_rename') {
        ...
        await user.save();
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.sendMessage(msg.chat.id, `Category renamed to "${normalizedName}".`, categoryKeyboardOptions(categories, user));
        return;
    }
    if (step === 'awaiting_budget_update') {
        ...
        const budgetCategoryId = String(user.state.payload.categoryId ?? '');
        ...
        user.state = { step: null, payload: {} };
        await user.save();
        await bot.sendMessage(msg.chat.id, `Budget updated to ${formatMoney(amount)}.`, {
    ...
    if (amount !== null) {
        if (!user.defaultCategoryId) {
            await bot.sendMessage(msg.chat.id, NOT_DEFAULT_CATEGORY_ERROR, { parse_mode: 'HTML' });
            return;
        }
        const category = await getCategoryById(user.defaultCategoryId, user.telegramUserId);
...
export async function handleCallback(bot, query, user) {
    ...
    if (data.startsWith('cat_type:')) {
        ...
        const payloadName = String(user.state.payload.name ?? '');
        await bot.sendMessage(chatId, `You chose ${type} category for "${payloadName}". Confirm?`, {
            reply_markup: {
                inline_keyboard: [
    ...
    if (data.startsWith('confirm_cat_type:')) {
        ...
        await user.save();
        await bot.editMessageReplyMarkup({ inline_keyboard: [] }, { chat_id: chatId, message_id: messageId });
        const budgetPayloadName = String(user.state.payload.name ?? '');
        await bot.sendMessage(chatId, `Great. Now set a budget for ${type === 'monthly' ? 'a monthly' : 'an annual'} "${budgetPayloadName}" category.`, { parse_mode: 'HTML' });
        return;
    }
    if (data.startsWith('default_yes:')) {
        ...
        await user.save();
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.sendMessage(chatId, DEFAULT_CATEGORY_SET_MESSAGE(category.name), { parse_mode: 'HTML', ...categoryKeyboardOptions(categories, user) });
    ...
    if (data.startsWith('history:')) {
        ...
        const text = category.type === 'monthly'
    ...
    if (data.startsWith('edit:')) {
        ...
        const isDefault = Boolean(user.defaultCategoryId &&
    ...
    if (data.startsWith('edit_reset:')) {
        ...
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.editMessageText(`Spend for "${category.name}" reset to 0.`, {
            chat_id: chatId,
            message_id: messageId,
        });
        await bot.sendMessage(chatId, 'Updated.', categoryKeyboardOptions(categories, user));
        return;
    }
    if (data.startsWith('edit_convert_to_monthly:')) {
        ...
        const categories = await getActiveCategories(user.telegramUserId);
        await bot.editMessageText(`"${category.name}" was converted to monthly.`, {
            chat_id: chatId,
            message_id: messageId,
        });
        await bot.sendMessage(chatId, 'Updated.', categoryKeyboardOptions(categories, user));
    ...
    if (data.startsWith('confirm_remove:')) {
        ...
        await bot.sendMessage(chatId, `"${category.name}" was removed from the menu.`, categoryKeyboardOptions(categories, user));
        return;
    }
