...
export function formatCategoryLine(category, user) {
    const spent = formatMoney(category.currentSpent);
    const budget = formatMoney(category.currentBudget);
    const isDefault = user.defaultCategoryId && String(user.defaultCategoryId) === String(category._id);
    return `${category.name} ${spent}/${budget}${isDefault ? ' ⭐' : ''}`;
}
export function formatCategoryDetails(category) {
...
export function formatAnnualHistory(category) {
    const budget = Number(category.currentBudget || 0);
    const monthly = category.currentYearMonthlySpent || new Map();
    ...
    const yearLines = [...(category.history?.years || [])]
        ...
        .map((item) => `${item.year} — ${formatMoney(item.spent)}/${formatMoney(item.budget)}`);
    const spent = Number(category.currentSpent || 0);
    const remaining = budget - spent;
...
export function mapEntries(mapLike) {
    if (!mapLike)
        return [];
    if (typeof mapLike === 'object' && 'entries' in mapLike && typeof mapLike.entries === 'function') {
        return Array.from(mapLike.entries()).map(([key, value]) => ({ key, value }));
    }
    return Object.entries(mapLike).map(([key, value]) => ({
        key,
        value,
    }));
}
