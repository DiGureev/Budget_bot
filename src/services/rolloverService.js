// src/services/rolloverService.js
...
export async function ensureAnnualCategoryCurrent(category, now) {
  ...
  if (currentYear !== now.year) {
    ...
    category.currentSpent = 0;
    category.period.year = now.year;
    category.currentYearMonthlySpent = {};
  }

  let map = category.currentYearMonthlySpent || {};
  const currentMonth = now.month;

  for (let m = 1; m <= currentMonth; m += 1) {
    const key = monthKey(now.year, m);
    const hasValue = map.get ? map.has(key) : Object.prototype.hasOwnProperty.call(map, key);
    if (!hasValue) {
      map.set ? map.set(key, 0) : (map[key] = 0);
    }
  }
