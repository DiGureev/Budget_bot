...
export function confirmCategoryTypeKeyboard(
  ...
  return {
    inline_keyboard: [
      ...
      [
        {
          text: `Set as ${capitalize(opposite)}`,
          callback_data: `cat_type:${opposite}`,
        },
      ],
    ],
  };
...
export function editCategoryKeyboard(category: ICategory, isDefault: boolean) {
  const rows: {text: string; callback_data: string}[][] = [
    ...
    [
      {
        text: "💰 Update budget",
        callback_data: `edit_budget:${String(category._id)}`,
      },
    ],
    [
      {
        text: "🔄 Reset spend to 0",
        callback_data: `edit_reset:${String(category._id)}`,
      },
    ],
  ];
  ...
  rows.push([
    {text: "⬅️ Back", callback_data: `open_category:${String(category._id)}`},
  ]);
