...
export function formatCategoryLine(category: ICategory, user: IUser): string {
  ...
  const isDefault =
    user.defaultCategoryId && String(user.defaultCategoryId) === String(category._id);
...
export function mapEntries(mapLike: Map<string, number> | Record<string, number> | undefined) {
  if (!mapLike) return [];
  ...
  return Object.entries(mapLike as Record<string, number>).map(([key, value]) => ({
    key,
    value,
  }));
}
